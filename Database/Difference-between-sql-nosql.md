...
- SQL 과 NoSQL 데이터베이스 간의 차이
  ...
  - SQL 과 NoSQL 의 스키마는 확연히 차이나는 부분임.  
  - [SQL](#sql관계형-데이터베이스) 과 [NoSQL](#nosql비관계형-데이터베이스) 에 대한 설명에서 간단하게 확인 할 수 있음.

  - 활용적인 측면
    - **SQL** 데이터베이스는 Join 이 많이 일어나면서 스키마 별로 제약 또는 레코드의 규칙 지정이 필요한 기업에서 사용하는 것이 좋고
    - **NoSQL** 데이터베이스는 스키마 사전 선언이 필요하지 않기 때문에 주로 정제되어 있지 않은 json , xml 형태의 데이터를 `collection` 에 넣어 사용하는 빅데이터에 사용하는 것이 좋음.
