...
#### 데이터 노드는 네임노드에게 어떻게 보고할까

**하트비트와 블록 리포트.**

- 하트비트는 데이터 노드가 네임노드와 계속해서 통신하고 있는지 확인하기 위한 메시지
- 블록 보고는 데이터 노드가 관리하는 블록 목록을 포함하여, 이를 통해 네임 노드는 HDFS 클러스터 전체에서 블록이 어디에 있는지를 파악할 수 있음.

**고가용성과 내결함성 관점에서의 하트비트와 블록 리포트.**

- 하트비트 : 데이터 노드가 정상적으로 작동하고 있는지 주기적으로 확인하는 매커니즘
  - 데이터 노드는 네임 노드에게 자신이 살아있음을 알리기 위해 메시지를 보냄. -> 노드의 상태를 알림.
    - IP 주소, 용량, 블록 보유 현황 etc.
  - 데이터 노드가 장애가 발생하여 하트비트를 전송하지 못하는 경우
    - 일정시간 동안 하트비트를 받지 못한 네임노드는 해당 노드를 장애로 분류한다.
    - 주변 데이터 노드들은 해당 노드의 저장된 블록을 가져와서 데이터 손실을 방지한다.(**고가용성**)
- 블록 리포트 : 데이터 노드가 관리하는 블록들을 네임노드에게 보내는 매커니즘
